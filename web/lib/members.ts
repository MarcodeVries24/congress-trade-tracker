...
/**
 * Quarterly buy/sell flow for one member, split by whether the disclosure met
 * its deadline.
 *
 * The lateness split is the part worth having. Across the corpus only 8.8% of
 * trades are filed late, so for most members it is absent — but it is not
 * evenly spread: Alan Armstrong filed 701 of 707 late, Julia Letlow 211 of
 * 257, while Pelosi's bars are solid throughout.
 */
export function getMemberTradeFlow(names: string[]): Promise<TradeFlowQuarter[]> {
  return getTradeFlow("t.member_name = ANY($1)", [names]);
}
